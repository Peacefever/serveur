...
def collect_sales():
	...
	de chaque boissons pour un jour donné
	'''
	datas = request.get_json()
	...
	for dictObject in datas['sales']:
		...
		sold_creation = db.execute("INSERT INTO sales (quantity_sales, day_sales, id_player, id_recipe) VALUES (%(quantity)s, %(day)s, %(p_id)s, %(r_id)s)",{"quantity":dictObject['quantity'],"day": currentDay, "p_id":playerID[0]['id_player'], "r_id":recipeID[0]['id_recipe']})
